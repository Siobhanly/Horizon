...
class ContentEnricher:
    ...
    async def _enrich_item(self, item: ContentItem) -> None:
        ...
        for lang in ("en", "zh"):
            ...
            if result.get(f"use_cases_{lang}"):
                val = result[f"use_cases_{lang}"]
                item.metadata[f"use_cases_{lang}"] = val.get("text") or str(val) if isinstance(val, dict) else str(val)
        ...
        item.metadata["use_cases"] = item.metadata.get("use_cases_en", "")
